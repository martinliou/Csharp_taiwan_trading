...
{
    /// <summary>
    /// Interaction logic for EditPostStock.xaml
    /// </summary>
    public partial class EditPostStock : UserControl
    {
        private bool complete = false;
        private bool abort = false;
        StockViewModel editedModel = new();
        public EditPostStock(StockViewModel model)
        {
            InitializeComponent();
            editedModel = model;
            MainWindow.mainWindow.mainContentControl.Content = MainGrid;

            string tradeDate = model.TradeDate;
            string ticks = model.Ticks;
            string[] ticksInArray = ticks.Split(',');
            StockNumber.Text = model.Symbol;
            TradeDate.Text = string.Format("{0}/{1}/{2}", tradeDate.Substring(0, 4), tradeDate.Substring(4, 2), tradeDate.Substring(6, 2));
            OpenAmp.Text = Convert.ToString(model.OpenAmp);
            OpenLeave.Text = Convert.ToString(model.OpenLeave);
            OpenBelowLeave.Text = Convert.ToString(model.OpenBelowLeave);
            ForceLeave.Text = Convert.ToString(model.ForceLeave);
            UpTicks.Text = Convert.ToString(ticksInArray.Length);
            Delay.Text = Convert.ToString(model.DelayTime);

            for (int i = 0; i < ticksInArray.Length; i++)
            {
                TextBox textBox = new();
                textBox.Name = string.Format("Tick_{0}", i);
                textBox.Width = 280;
                textBox.Text = Convert.ToString(ticksInArray[i]);
        ...
        private void DbConfirmClick(object sender, System.Windows.RoutedEventArgs e)
        {
            ...
            if (conn.IsConnect())
                conn.UpdateSetting(insertedData);
